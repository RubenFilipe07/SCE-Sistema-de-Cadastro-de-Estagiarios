...
app.post('/cadastro', (req, res) => {
    ...
    pool.query('INSERT INTO projetos (nome, cnpj, email, senha) VALUES ($1, $2, $3, $4)', [nome, cnpj, email, senha ], (err, result) => {
        if (err) {
...
app.post('/login', (req, res) => {
    ...
    pool.query('SELECT id, nome, email FROM projetos WHERE email = $1 AND senha = $2', [email, senha], (err, result) => {
      if (err) {
        res.status(500).send(err.toString());
    ...
    app.post('/estagiarios', (req, res) => {
        const { nome, cpf, email, telefone, curso, periodo, instituicao, cargaHoraria } = req.body;
        pool.query('INSERT INTO estagiarios (nome, cpf, email, telefone, curso, periodo, instituicao, cargaHoraria) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', [nome, cpf, email, telefone, curso, periodo, instituicao, cargaHoraria ], (err, result) => {
            if (err) {
                res.status(500).send(err.toString());
            } else {
                res.json({ status: 'success', message: 'Cadastrado com sucesso!' });
    ...
    app.put('/estagiarios/:id', (req, res) => {
        const id = req.params.id;
        const { nome, cpf, email, telefone, curso, periodo, instituicao, cargaHoraria } = req.body;
        pool.query('UPDATE estagiarios SET nome = $1, cpf = $2, email = $3, telefone = $4, curso = $5, periodo = $6, instituicao = $7, cargaHoraria = $8 WHERE id = $9', [nome, cpf, email, telefone, curso, periodo, instituicao, cargaHoraria, id], (err, result) => {
            if (err) {
                res.status(500).send(err.toString());
            } else {
